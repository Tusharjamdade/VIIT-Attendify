...
export default function SubjectCard({
  ...
  // New state to store class location coordinates
  const [classLatitude, setClassLatitude] = useState(0);
  const [classLongitude, setClassLongitude] = useState(0);

  const { colors } = useTheme(); // Get current theme colors

  // Fetch class location from Firestore
  const fetchClassLocation = async () => {
    try {
      const classLocationDocRef = doc(firestore, 'classLocation', 'U6rrz1w3nuVE9rla9vUf');
      const docSnapshot = await getDoc(classLocationDocRef);

      if (docSnapshot.exists()) {
        const data = docSnapshot.data();
        setClassLatitude(data.classLatitude);
        setClassLongitude(data.classLongitude);
        setDistanceRadius(data.distance);
        console.log(JSON.stringify(data));
      } else {
        console.error('Class location document not found.');
      }
    } catch (error) {
  ...
  useEffect(() => {
    fetchClassLocation();
    checkAttendance();
  }, [subjectId]); // This will only run when the subjectId changes, avoiding infinite loop
  ...
  const markAttendance = async () => {
    ...
    const { latitude, longitude } = location;
    const distance = getDistance(latitude, longitude, classLatitude, classLongitude);
    console.log(classLatitude, classLongitude, distanceRadius);
