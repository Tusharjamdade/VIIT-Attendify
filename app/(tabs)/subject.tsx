...
import { Ionicons } from '@expo/vector-icons';
import { useRouter } from 'expo-router';
import { firestore, auth } from '../../src/firebase'; // Ensure you initialize auth and firestore properly
import {
  deleteDoc,
  doc,
  collection,
...
const Subject = () => {
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            setIsDeleting(true);
            try {
              const password = 'YourPassword'; // Replace with secure password handling
              const credential = EmailAuthProvider.credential(
                currentUser.email,
                password
              );
  ...
  const imageUrl =
    currentUser?.image && imageMap[currentUser.image]
      ? imageMap[currentUser.image]
